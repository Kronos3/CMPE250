...
; Compute F = P + 3Q + 65, G = 3P - 2Q + 12, and Result = F + G with byte overflow checking
;Name:  Andrei Tumbar
;Date:  9/13/2020
;Class:  CMPE-250
...
; G = 3P -2Q + 12
            LDR     R1,=P
            LDR     R1,[R1,#0]  ; Load P into R1
            LDR     R2,=Q
            LDR     R2,[R2,#0]  ; Load Q into R2
            
            ; Compute 3Q and store into R3
            LSLS    R0,R2,#1    ; R0 = 2Q
            ADDS    R0,R0,R2    ; R0 = 2Q + Q = 3Q
            BL      CHECKOVER   ; Check if R0 overflows
            BVS     FOVERFLOW
            
            ADDS    R0,R0,R1    ; R0 = P + 3Q
            BL      CHECKOVER   ; Check if R0 overflows
            BVS     FOVERFLOW
            
            LDR     R3,=const_F
            LDR     R3,[R3,#0]  ; R3 = const_F
            ADDS    R0,R0,R3    ; R0 = P + 3Q + 65
            BL      CHECKOVER   ; Check if R0 overflows
            BVS     FOVERFLOW
            BL      FSTOP       ; Finished with F calculation
            
FOVERFLOW   MOVS    R0,#0       ; Overflow occured, clear the result
FSTOP       LDR     R3,=F
            STR     R0,[R3,#0]  ; Store the result 'R0' into F
            ...
            LSLS    R3,R2,#1    ; R3 = 2Q
            PUSH    {R0}        ; Save the old R0 so we can use R3 in CHECKOVER
            MOVS    R0,R3       ; Init R0 as the input param for CHECKOVER
            BL      CHECKOVER
            BVS     GOVERFLOW
            POP     {R0}        ; Restore R0
            
            SUBS    R0,R0,R3    ; R0 = 3P - 2Q
            BL      CHECKOVER
            BVS     GOVERFLOW
            
            LDR     R3,=const_G
            LDR     R3,[R3,#0]  ; R3 = const_G
            ADDS    R0,R0,R3    ; R0 = 3P - 2Q + 12
            BL      CHECKOVER
            BVS     GOVERFLOW
            BL      GSTOP       ; Finished with F calculation

GOVERFLOW   MOVS    R0,#0       ; Overflow occured, clear the result
GSTOP       LDR     R3,=G
            STR     R0,[R3,#0]  ; Store the result 'R0' into G
            
            ; Compute Result = F + G
            LDR     R1,=F
            LDR     R1,[R1,#0]  ; Load F into R1
            LDR     R2,=G
            LDR     R2,[R2,#0]  ; Load G into R2
            ADDS    R0,R1,R2    ; Compute Result
            BL      CHECKOVER   ; Check if result overflows
            BVS     RESOVER
            BL      RESSTOP

RESOVER     MOVS    R0,#0       ; Result overflows, set it to 0
RESSTOP     LDR     R3,=Result
            STR     R0,[R3,#0]  ; Load the R0 into the result variable
...
CHECKOVER   PROC    {R0}; Check if R0 is in range and set flags
            PUSH    {R0}
            CMP     R0,#127
            BGT     OVERFLOW
            SUBS    R0,R0,#0xFF
...
OVERFLOW    PROC    {R0}
            LDR     R0,=0x7FFFFFFF
            ADDS    R0,R0,#1        ; Set the overflow flag
            BX      LR
            ENDP
